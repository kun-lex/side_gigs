...
const Main = () => {
    ...
    const handleClick = () => {
        setIsIndigo((prevState) => !prevState);
    }
    return(
        <div>
            <div className="absolute inset-0 z-0 group cursor-pointer bg-violet-50">
                <canvas class="canvas3d" className="block w-full h-full relative z-10"></canvas>
                
                <div className={`absolute left-1/2 top-1/2 z-0 -translate-x-1/2 -translate-y-1/2 {isIndigo ? 'bg-indigo-600' : 'bg-lime-500'}` }>
                ...
                <button onClick={handleClick} type="button" id="rotate" className="absolute left-1/2 bottom-10 -translate-x-1/2 z-20 bg-indigo-600 rounded-full pointer-events-none group-hover:scale-110 transition">
